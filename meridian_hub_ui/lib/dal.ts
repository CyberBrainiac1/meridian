...
export const getHubState = cache(async (): Promise<HubState> => {
  ...
  const [profileResult, requestResult, promptResult, medicationResult] = await Promise.all([
    ...
    supabase.from("resident_hub_medication_feed").select("*").order("scheduled_for", { ascending: true }).returns<HubMedicationDose[]>(),
  ]);

  if (profileResult.error || !profileResult.data) redirect("/login?error=not_a_provisioned_hub");
  if (requestResult.error || promptResult.error || medicationResult.error) {
  ...
  return {
    ...
    medicationDoses: medicationResult.data ?? [],
  };
});
