...
enum HubActionService {
    ...
    static func createAssistanceRequest(_ kind: AssistanceKind) async -> HubActionOutcome {
        await bounded {
            do {
...
enum HubCopy {
    ...
    static func eta(for request: HubAssistanceRequest) -> Eta {
        let phrase = MeridianFormat.approxMinutes(request.etaSeconds)
...
final class HubSurfaceViewModel: ObservableObject {
    ...
    func submitRequest(_ kind: AssistanceKind) async {
    ...
    private func setDoseStatus(_ id: String, _ status: MedicationDoseStatus) {
    ...
    private func setActivityStatus(_ id: String, _ status: ActivityCompletionStatus) {
