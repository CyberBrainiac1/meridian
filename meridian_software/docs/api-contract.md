...
| Screen | Source | Notes |
| --- | --- | --- |
| Linked resident name | `select * from family_linked_residents` | Added 2026-07 — none of the other family-scoped views/tables carry a display name (family has no RLS access to `people`/`resident_profiles`), so this is the only source for "Maggie" in copy like "Maggie had a good day." Name only, no room/risk_flags/care_notes. |
