...
struct AlertFeedView: View {
    /// Owned by CareRootView (shared with the urgent-alert overlay), not this
    ...
    /// Also owned by CareRootView, and deliberately the *same* instance the
    ...
    /// Set by CareRootView's "View alert" action to jump straight into an
        ...
        NavigationStack(path: $path) {
            ...
            .toolbar {
                ToolbarItem(placement: .topBarTrailing) {
                    CareHelpToolbarButton(topic: .alerts)
                }
            }
    ...
    private func respond(to request: AssistanceRequest, newStatus: CareAssistanceStatus) {
...
private struct AssistanceRequestRow: View {
    ...
    let onRespond: (CareAssistanceStatus) -> Void

    /// Mirrors CareIncidentStatus.nextStatuses: only offer transitions
    /// respond_to_assistance_request will actually accept.
    private var offeredStatuses: [CareAssistanceStatus] {
...
/// Care-facing wording for CareAssistanceStatus. Kept here rather than on the
/// model in Domain/Types.swift, which is a straight mirror of the SQL schema
/// and stays free of app copy.
private extension CareAssistanceStatus {
