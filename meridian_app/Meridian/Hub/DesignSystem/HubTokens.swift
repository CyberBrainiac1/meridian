...
// The Hub used to declare its own `extension Color { init(hex:alpha:) }` here.
// It was byte-for-byte identical to Care's (DesignSystem/Tokens.swift), and two
// identical extensions on Color in one module is a redeclaration error, so this
// one is gone and Care's serves all three interfaces. The token *values* above
// stay Hub-only — see the header comment.
