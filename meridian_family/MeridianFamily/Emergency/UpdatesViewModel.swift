...
final class UpdatesViewModel: ObservableObject {
    @Published private(set) var notifications: [NotificationRow] = []
    ...
    func load() async {
        do {
            let rows: [NotificationRow] = try await SupabaseManager.client
                .from("notifications")
                .select()
    ...
    private func fireLocalNotification(for row: NotificationRow) {
