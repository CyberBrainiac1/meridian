...
// ---------------------------------------------------------------------------
// Everything two compositions can share lives here: tokens, the two easing
// helpers, every inline SVG path, the design-system primitives, and the app
// screens themselves. `MeridianDemo` (29s reel) and `MeridianExplainer` (2min
// standalone) both import from this file so a correction to a screen, a glyph
// or a colour lands in both cuts at once. Nothing in here reads a global
// frame clock: every screen takes the frame it should animate on as a prop,
// which is what lets the explainer play the same beat at a different pace
// without forking it.
// ---------------------------------------------------------------------------
...
export const clamp = (value: number, input: number[], output: number[]) => interpolate(value, input, output, {extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: ease});
...
export const ramp = (value: number, input: number[], output: number[]) => interpolate(value, input, output, {extrapolateLeft: "clamp", extrapolateRight: "clamp"});
...
export const svgGlyph = (children: React.ReactNode, color: string, size: number, strokeWidth = 1.8) => (
...
export const GLYPH_CAMERA = <><path d="M14.5 5h-5L7.5 8H4a1.5 1.5 0 0 0-1.5 1.5v9A1.5 1.5 0 0 0 4 20h16a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 20 8h-3.5L14.5 5Z" /><circle cx={12} cy={13.2} r={3.4} /></>;
export const GLYPH_CPU = <>
...
export const GLYPH_ACTIVITY = <path d="M3 12.5h4l2.2-7.5 4 15 2.2-7.5H21" />;
export const GLYPH_BELL = <><path d="M6.5 8.5a5.5 5.5 0 0 1 11 0c0 4.6 1.8 5.7 1.8 5.7H4.7s1.8-1.1 1.8-5.7Z" /><path d="M10.3 18.2a1.9 1.9 0 0 0 3.4 0" /></>;
// The explainer's state-machine panel draws its transitions as an arrow path,
// never U+2192: an arrow is exactly the class of decorative Unicode that font
// falls back in headless Chromium.
export const GLYPH_ARROW_RIGHT = <><path d="M4 12h16" /><path d="m14 6 6 6-6 6" /></>;
...
export const L_HEART_HANDSHAKE = <path d="M19.414 14.414C21 12.828 22 11.5 22 9.5a5.5 5.5 0 0 0-9.591-3.676.6.6 0 0 1-.818.001A5.5 5.5 0 0 0 2 9.5c0 2.3 1.5 4 3 5.5l5.535 5.362a2 2 0 0 0 2.879.052 2.12 2.12 0 0 0-.004-3 2.124 2.124 0 1 0 3-3 2.124 2.124 0 0 0 3.004 0 2 2 0 0 0 0-2.828l-1.881-1.882a2.41 2.41 0 0 0-3.409 0l-1.71 1.71a2 2 0 0 1-2.828 0 2 2 0 0 1 0-2.828l2.823-2.762" />;
export const L_PHONE_CALL = <><path d="M13 2a9 9 0 0 1 9 9" /><path d="M13 6a5 5 0 0 1 5 5" /><path d="M13.832 16.568a1 1 0 0 0 1.213-.303l.355-.465A2 2 0 0 1 17 15h3a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2A18 18 0 0 1 2 4a2 2 0 0 1 2-2h3a2 2 0 0 1 2 2v3a2 2 0 0 1-.8 1.6l-.468.351a1 1 0 0 0-.292 1.233 14 14 0 0 0 6.392 6.384" /></>;
export const L_BELL_RING = <><path d="M10.268 21a2 2 0 0 0 3.464 0" /><path d="M22 8c0-2.3-.8-4.3-2-6" /><path d="M3.262 15.326A1 1 0 0 0 4 17h16a1 1 0 0 0 .74-1.673C19.41 13.956 18 12.499 18 8A6 6 0 0 0 6 8c0 4.499-1.411 5.956-2.738 7.326" /><path d="M4 2C2.8 3.7 2 5.7 2 8" /></>;
export const L_BELL = <><path d="M10.268 21a2 2 0 0 0 3.464 0" /><path d="M3.262 15.326A1 1 0 0 0 4 17h16a1 1 0 0 0 .74-1.673C19.41 13.956 18 12.499 18 8A6 6 0 0 0 6 8c0 4.499-1.411 5.956-2.738 7.326" /></>;
export const L_SHIELD_CHECK = <><path d="M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z" /><path d="m9 12 2 2 4-4" /></>;
export const L_USER_ROUND_CHECK = <><path d="M2 21a8 8 0 0 1 13.292-6" /><circle cx="10" cy="8" r="5" /><path d="m16 19 2 2 4-4" /></>;
export const L_CHECK_CIRCLE = <><path d="M21.801 10A10 10 0 1 1 17 3.335" /><path d="m9 11 3 3L22 4" /></>;
export const L_TRIANGLE_ALERT = <><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3" /><path d="M12 9v4" /><path d="M12 17h.01" /></>;
export const L_CLOCK = <><circle cx="12" cy="12" r="10" /><path d="M12 6v6l4 2" /></>;
export const L_PILL = <><path d="m10.5 20.5 10-10a4.95 4.95 0 1 0-7-7l-10 10a4.95 4.95 0 1 0 7 7Z" /><path d="m8.5 8.5 7 7" /></>;
export const L_UTENSILS = <><path d="M3 2v7c0 1.1.9 2 2 2h4a2 2 0 0 0 2-2V2" /><path d="M7 2v20" /><path d="M21 15V2a5 5 0 0 0-5 5v6c0 1.1.9 2 2 2h3Zm0 0v7" /></>;
export const L_FOOTPRINTS = <><path d="M4 16v-2.38C4 11.5 2.97 10.5 3 8c.03-2.72 1.49-6 4.5-6C9.37 2 10 3.8 10 5.5c0 3.11-2 5.66-2 8.68V16a2 2 0 1 1-4 0Z" /><path d="M20 20v-2.38c0-2.12 1.03-3.12 1-5.62-.03-2.72-1.49-6-4.5-6C14.63 6 14 7.8 14 9.5c0 3.11 2 5.66 2 8.68V20a2 2 0 1 0 4 0Z" /><path d="M16 17h4" /><path d="M4 13h4" /></>;
export const L_CHECK = <path d="M20 6 9 17l-5-5" />;
export const L_CLIPBOARD_LIST = <><rect width="8" height="4" x="8" y="2" rx="1" ry="1" /><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2" /><path d="M12 11h4" /><path d="M12 16h4" /><path d="M8 11h.01" /><path d="M8 16h.01" /></>;
export const L_USERS = <><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" /><path d="M16 3.128a4 4 0 0 1 0 7.744" /><path d="M22 21v-2a4 4 0 0 0-3-3.87" /><circle cx="9" cy="7" r="4" /></>;
export const L_SETTINGS = <><path d="M9.671 4.136a2.34 2.34 0 0 1 4.659 0 2.34 2.34 0 0 0 3.319 1.915 2.34 2.34 0 0 1 2.33 4.033 2.34 2.34 0 0 0 0 3.831 2.34 2.34 0 0 1-2.33 4.033 2.34 2.34 0 0 0-3.319 1.915 2.34 2.34 0 0 1-4.659 0 2.34 2.34 0 0 0-3.32-1.915 2.34 2.34 0 0 1-2.33-4.033 2.34 2.34 0 0 0 0-3.831A2.34 2.34 0 0 1 6.35 6.051a2.34 2.34 0 0 0 3.319-1.915" /><circle cx="12" cy="12" r="3" /></>;
export const L_SUN = <><circle cx="12" cy="12" r="4" /><path d="M12 2v2" /><path d="M12 20v2" /><path d="m4.93 4.93 1.41 1.41" /><path d="m17.66 17.66 1.41 1.41" /><path d="M2 12h2" /><path d="M20 12h2" /><path d="m6.34 17.66-1.41 1.41" /><path d="m19.07 4.93-1.41 1.41" /></>;
export const L_DOOR_OPEN = <><path d="M11 20H2" /><path d="M11 4.562v16.157a1 1 0 0 0 1.242.97L19 20V5.562a2 2 0 0 0-1.515-1.94l-4-1A2 2 0 0 0 11 4.561z" /><path d="M11 4H8a2 2 0 0 0-2 2v14" /><path d="M14 12h.01" /><path d="M22 20h-3" /></>;
export const L_LOCK = <><circle cx="12" cy="16" r="1" /><rect x="3" y="10" width="18" height="12" rx="2" /><path d="M7 10V7a5 5 0 0 1 10 0v3" /></>;
export const L_ACTIVITY = <path d="M22 12h-2.48a2 2 0 0 0-1.93 1.46l-2.35 8.36a.25.25 0 0 1-.48 0L9.24 2.18a.25.25 0 0 0-.48 0l-2.35 8.36A2 2 0 0 1 4.49 12H2" />;
export const L_WIFI_OFF = <><path d="M12 20h.01" /><path d="M8.5 16.429a5 5 0 0 1 7 0" /><path d="M5 12.859a10 10 0 0 1 5.17-2.69" /><path d="M19 12.859a10 10 0 0 0-2.007-1.523" /><path d="M2 8.82a15 15 0 0 1 4.177-2.643" /><path d="M22 8.82a15 15 0 0 0-11.288-3.764" /><path d="m2 2 20 20" /></>;

// The brand shield: M-notched, with the medical cross. NOT the MeridianHub app
// icon (house glyph) -- that is a product icon, not the brand.
export const BrandShield = ({size, tint, stroke, style}: {size: number; tint: string; stroke: number; style?: React.CSSProperties}) => (
  <svg width={size} height={size} viewBox="0 0 1024 1024" aria-label="Meridian" style={style}>
    <path d="M242 250 L420 196 L512 320 L604 196 L782 250 L782 540 C782 694 668 800 512 858 C356 800 242 694 242 540 Z"
          fill="none" stroke={tint} strokeWidth={stroke} strokeLinejoin="round" strokeLinecap="round" />
    <path d="M462 400 H562 V470 H632 V570 H562 V640 H462 V570 H392 V470 H462 Z" fill={tint} />
  </svg>
);

// Persistent brand bug, top right. Sits above every beat so the cut is
...
// full size. `darkEnd` and `fade` differ per composition, which is the only
// reason they are props.
export const BrandMark = ({darkEnd, fade}: {darkEnd: number; fade: [number, number, number, number]}) => {
  const f = useCurrentFrame();
  const onDark = f < darkEnd;
  const tint = onDark ? C.mint : C.primary;
  // Fade out just before the end card takes over, so the two marks never
  // appear at once.
  const opacity = ramp(f, fade, [0, 1, 1, 0]) * (onDark ? 0.92 : 0.8);
  return (
    <div style={{position: "absolute", top: 46, right: 56, zIndex: 20, opacity,
                 display: "flex", alignItems: "center", gap: 12, pointerEvents: "none"}}>
      {/* Same mark as the end card and the deck title slide. */}
      <BrandShield size={34} tint={tint} stroke={62} style={{display: "block",
...
export const SHADOW_SOFT = "0 1px 2px rgba(12,74,110,.04), 0 8px 24px rgba(12,74,110,.08)";
export const Card = ({children, style}: {children: React.ReactNode; style?: React.CSSProperties}) => <div style={{background: C.surface, border: `1px solid ${C.border}`, borderRadius: 20, boxShadow: SHADOW_SOFT, ...style}}>{children}</div>;
...
export const Badge = ({tone, children, size = 22}: {tone: keyof typeof BADGE; children?: React.ReactNode; size?: number}) => {
  const t = BADGE[tone];
...
export type Tab = {key: string; label: string; glyph: React.ReactNode; badge?: boolean};
export const TabBar = ({tabs, active, activeColor}: {tabs: Tab[]; active: string; activeColor?: (key: string) => string}) => (
...
export const ActionButton = ({label, note, glyph, bg, color, border, style, children}: {label: string; note: string; glyph: React.ReactNode; bg: string; color: string; border?: string; style?: React.CSSProperties; children?: React.ReactNode}) => (
...
export const Ripple = ({size, opacity, color = "rgba(255,255,255,.7)"}: {size: number; opacity: number; color?: string}) => size <= 0 ? null : <span style={{position: "absolute", left: "50%", top: "50%", marginLeft: -size / 2, marginTop: -size / 2, width: size, height: size, borderRadius: "50%", border: `3px solid ${color}`, opacity, pointerEvents: "none"}} />;
// The app name outranks the tagline: it is the thing the audience must retain.
export const AppName = ({children}: {children: React.ReactNode}) => <div style={{fontSize: 84, fontWeight: 880, color: C.appName, letterSpacing: -3.2, lineHeight: 1}}>{children}</div>;
...
export const AudienceLine = ({children}: {children: React.ReactNode}) => <div style={{marginTop: 16, paddingLeft: 18, borderLeft: `6px solid ${C.primaryAlt}`, fontSize: 38, fontWeight: 830, color: C.primary, letterSpacing: -0.4}}>{children}</div>;
// The causal thread. Each beat's sub-line states how it follows from the
// fall the viewer just watched, so the cut reads as one story instead of
// four unrelated screens.
export const StoryLine = ({children}: {children: React.ReactNode}) => <div style={{marginTop: 30, fontSize: 31, color: C.foregroundMuted, lineHeight: 1.4}}>{children}</div>;

export const Device = ({children, label}: {children: React.ReactNode; label: string}) => <div aria-label={label} style={{width: 560, height: 970, padding: 15, background: "#102833", borderRadius: 64, boxShadow: "0 40px 90px rgba(0,0,0,.28)", border: "3px solid #31505d"}}>
...
export const Background = ({children, opacity = 1}: {children: React.ReactNode; opacity?: number}) => <AbsoluteFill style={{background: `radial-gradient(circle at 18% 18%, #dff5ff 0, transparent 32%), radial-gradient(circle at 84% 82%, #d9fbef 0, transparent 30%), ${C.background}`, color: C.foreground, overflow: "hidden", opacity}}>{children}</AbsoluteFill>;

export const FullVideo = ({src, startFrom, scale = 1, filter, playbackRate}: {src: string; startFrom: number; scale?: number; filter?: string; playbackRate?: number}) => <OffthreadVideo src={staticFile(src)} startFrom={startFrom} playbackRate={playbackRate} style={{width: "100%", height: "100%", objectFit: "cover", scale, filter}} />;

// The corner falloff that keeps the caption legible over the footage.
export const Vignette = () => <AbsoluteFill style={{background: "radial-gradient(circle, transparent 44%, rgba(0,0,0,.32))"}} />;
...
// phone screens) follow: the video stays, the alert travels.
export const DarkCaption = ({opacity, y, privacy, bridge, bridgeText = "Only the alert does. Here is where it goes."}: {opacity: number; y: number; privacy: number; bridge: number; bridgeText?: string}) => (
  <div style={{position: "absolute", left: 120, bottom: 150, translate: `0 ${y}px`, opacity}}>
    ...
    <div style={{opacity: bridge, marginTop: 16, marginLeft: 5, color: C.mint, fontSize: 37, fontWeight: 700, letterSpacing: -0.5, padding: "0 30px"}}>{bridgeText}</div>
  </div>
);

// startFrom=58 on both clips. Measured on the source: she walks in from clip
// frame 0, the stumble starts at clip frame ~90, the impact is over by ~115
// and the pose is settled by ~120; frames 211-291 are the held tail. At 1x
// that leaves the reel 32 frames of upright walking for the skeleton to lock
// on in front of the audience before the fall starts.
export const CLIP_START = 58;

// ---------------------------------------------------------------------------
// The pipeline diagram. Ported from the imported Claude Design animation
// (meridian-scene.jsx, NODES) -- its node styling, labels and subs.
//
...
// decode our clip. Remotion decodes it fine, so the pose on screen in both
// cuts is the real model output from skeleton.mp4. Baking in a picture of a
// skeleton would fake the one thing in this film that is genuinely ours.
// ---------------------------------------------------------------------------
export const PIPE_X = [220, 700, 1180, 1660];
export const PIPE_Y = 660;
export const PIPE_NODES = [
  {at: 8, label: "CAMERA", sub: "Always watching, right inside the building", glyph: GLYPH_CAMERA},
  // "17-point" stays numeric. The reel's no-digits rule targets METRICS a judge
  // would ask us to source -- latency, room counts, ARR. This is a product
...
export const PIPE_CHIPS: [string, number][] = [["Normal", 66], ["Candidate", 76], ["Confirmed", 86]];
// Connectors and nodes only. The caller owns the plate, the pose layer under
// it and the beat envelope, because the reel and the explainer hold this
// diagram for very different lengths of time.
export const PipelineDiagram = ({local}: {local: number}) => {
...
export const IncidentRow = ({tone, when, copy, status, statusColor, children, style}: {tone: keyof typeof BADGE; when: string; copy: string; status: string; statusColor?: string; children?: React.ReactNode; style?: React.CSSProperties}) => (
...
// The caregiver phone. `local` is its own action clock, so the explainer can
// run the same screen at half speed by feeding it a warped frame.
export const CareDevice = ({local, style}: {local: number; style?: React.CSSProperties}) => {
  ...
  return <div style={{...style, translate: `0 ${phoneY}px`}}><Device label="MeridianCare caregiver alerts">
...
// MeridianHub
// ---------------------------------------------------------------------------
// Shared Hub chrome. The help and visitor beats are the same physical device on
// stage, so they share the frame and only the card inside it changes -- which
// is exactly what the in-device swap has to sell.
...
export const HubFrame = ({title, sub, bgOpacity, contentOpacity, panelX, children}: {title: React.ReactNode; sub: React.ReactNode; bgOpacity: number; contentOpacity: number; panelX: number; children: React.ReactNode}) => (
...
// The kiosk card stack: the three-way choice, then what the kiosk does once one
// is pressed. `local` is its own action clock.
export const HubHelpPanel = ({local}: {local: number}) => {
  ...
  // from zero, leaving the panel visually empty for most of a second (measured:
  // card ink fell from 182k to 15k). A real kiosk repaints instantly too. Only
  // a small scale pop marks the arrival.
    ...
    {requested && <>
      ...
      <div style={{marginTop: 20, padding: 22, background: C.surface, border: `3px solid ${C.border}`, borderRadius: 20,
        ...
        {/* .eta. The shipped surface really does derive this from recent
            ...
            honest -- but its figure is a digit, and a fixed ETA number is a
            forbidden claim in both cuts, so the derived estimate is stated in
            words and the sub-line keeps naming what it rests on. */}
...
export const HubVisitorPanel = ({local}: {local: number}) => {
...
export const FamilySection = ({children, style}: {children: React.ReactNode; style?: React.CSSProperties}) => (
  <div style={{fontSize: 20, fontWeight: 600, color: C.foregroundMuted, textTransform: "uppercase", letterSpacing: 1, ...style}}>{children}</div>
);
// The family phone shows both of the Family kit's load-bearing tabs, because
// the mandated content corrections land on both: Today first -- the calm daily
// baseline -- then a real tab change to Updates, where the alert and the staff
// response arrive. The tab bar does the transition, as it does in the app.
export const FamilyDevice = ({local, style}: {local: number; style?: React.CSSProperties}) => {
  ...
  return <div style={{...style, translate: `0 ${phoneY}px`}}><Device label="MeridianFamily daily summary and updates">
    {!onUpdates && (
...
// ---------------------------------------------------------------------------
// The closer, matching the deck's title slide: M-notched shield with a medical
// cross, MERIDIAN in caps, the deck's subtitle. The reel runs it for one second
// flat, because stage time belongs to the product and the frame only needs to
// match the slide already on screen beside it. The standalone explainer holds
// it and adds a footer line, so both are props.
// ---------------------------------------------------------------------------
const DECK_BLUE = "#69A8E0";
const DECK_TEXT_BLUE = "#6B96EE";
export const EndCard = ({footer, footerIn = [40, 64]}: {footer?: React.ReactNode; footerIn?: [number, number]}) => {
  const f = useCurrentFrame();
  const o = ramp(f, [0, 9], [0, 1]);
  // The beat before this one runs its own fade to this exact navy, and it is
  // still running underneath for the first two frames of the end card. The
  // plate only takes over on the frame after that beat unmounts, which keeps
  // the fade one continuous linear ramp instead of stacking two of them.
  const plate = ramp(f, [1, 2], [0, 1]);
  const footerOpacity = ramp(f, footerIn, [0, 1]);
  return <AbsoluteFill style={{justifyContent: "center", alignItems: "center", display: "flex"}}>
    ...
    <div style={{textAlign: "center", opacity: o, position: "relative"}}>
      <BrandShield size={120} tint={DECK_BLUE} stroke={52} style={{display: "block", margin: "0 auto 26px"}} />
      ...
      {footer && <div style={{fontSize: 34, fontWeight: 600, marginTop: 46, color: DECK_TEXT_BLUE, letterSpacing: -0.4, opacity: footerOpacity}}>{footer}</div>}
    </div>
  </AbsoluteFill>;
};
